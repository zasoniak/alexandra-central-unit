    ...
    class FirebaseGadgetObserver implements Observer {
        ...
        public void pushCurrentState() {
            ...
            switch(gadgetReference.getType())
            {
                case WallSocket:
                {
                    currentStateReference.child(gadgetReference.getId().toString()).child(Socket.IS_ON).setValue(((MultiSocket) gadgetReference).isOn());
                    for(int i = 0; i < ((MultiSocket) gadgetReference).getSocketNumber(); i++)
                    {
                        currentStateReference.child(gadgetReference.getId().toString()).child(String.valueOf(i)).child(Socket.IS_ON).setValue(((MultiSocket) gadgetReference).getChannelOn(i));
